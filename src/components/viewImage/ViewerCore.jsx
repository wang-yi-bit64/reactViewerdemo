...
class ViewerCore extends Component {
  ...
  componentDidMount() {
  	const core = this.viewerCore.current
  	core.addEventListener("tranditionend", this.handleTransitionEnd, false)(this.viewerCore.current).addEventListener("wheel", this.handleMouseScroll, false)
  ...
  getImgWidthHeight(imgWidth, imgHeight) {
  	let width = 0
  	let height = 0
  	const maxWidth = this.containerWidth * 0.8
  	const maxHeight = (this.containerHeight - this.footerHeight) * 0.8
  ...
  loadImgSuccess = (activeImage, imgWidth, imgHeight, isNewImage) => {
  	...
  	const top = (this.containerHeight - height - this.footerHeight) / 2
  ...
  handleZoom = (targetX, targetY, direct, scale) => {
  	...
  	if (this.state.width === 0) {
  		...
  		const directX = this.state.scaleX > 0 ? 1 : -1
  		const directY = this.state.scaleY > 0 ? 1 : -1
  ...
  handleResize = () => {
  	...
  	if (this.props.visible) {
  		const left = (this.containerWidth - this.state.width) / 2
  		const top = (this.containerHeight - this.state.height) / 2
  		this.setState({
  			left: left,
  			top: top,
  		})
